function [ImageIn_pn,ImageOut,CompStats,CompConCs]=ElecEnterpsCompletion(ImageIn,origin,ratio1,ratio2,ratio3,th,T0,Te,conn)
...
%CompStats：输入ImageIn的颅骨边界上的小连通分量（可能属于电极，也可能是残余颅骨干扰）seed0的统计信息，是一个n*3的矩阵，n是小连通分量的数目
%CompStats(:,1)是每个颅骨边界上的小连通分量seed0在输入ImageIn中的连通分量编号
%CompStats(:,2)是对输入ImageIn的每个颅骨边界上的小连通分量seed0是否属于电极的判据，值为1表明“是”，值为0表明“否”
%CompStats(:,3)是输入ImageIn的每个颅骨边界上的小连通分量seed0区域生长的阈值终值
