function [ImageOut,InCs_stats,InCs_grow]=AllInnerConCsGrow4(origin,basis,basis_S,elecenters_n,elecenters_sub,th,ratio,T0,Te,fignum)
...
IntracConC=basis_L;
for i=1:ICnum
    if isempty(intersect(InnerConC_L,i))
        IntracConC(basis_L==i)=0;
    end
end
IntracConC=logical(IntracConC);
NonIntracConC=logical(basis-IntracConC);
        ...
        if InCi_judge==1
            for j=(i+1):ICnum
                ...
                if length(IS)/length(InnerConCj_ind)>0.8
                    InnerConCi_ind=find(InnerConCi);
                    [r1,c1,s1]=ind2sub(size(InnerConCi),InnerConCi_ind);
                    [r2,c2,s2]=ind2sub(size(InnerConCj),InnerConCj_ind);
                    [r3,c3,s3]=ind2sub(size(InCi_grow),InCi_grow_ind);
                    InnerConCi_sub=[r1,c1,s1];
                    InnerConCj_sub=[r2,c2,s2];
                    InCi_grow_sub=[r3,c3,s3];
                    InnerConCi_centersub=mean(InnerConCi_sub,1);
                    InnerConCj_centersub=mean(InnerConCj_sub,1);
                    for k=1:elecenters_n
                        IS=intersect(InCi_grow_sub,elecenters_sub{k},'rows');
                        if size(IS,1)/size(elecenters_sub{k},1)>0
                            electi_sub=elecenters_sub{k};
                        end
                    end
                    electi_centersub=mean(electi_sub,1);
                    P=electi_centersub;
                    Q=InnerConCi_centersub;
                    C=InnerConCj_centersub;
                    [linePQ_sub,~]=MakeLine(P,Q);
%                     figure(fignum-2);plot3(P(2),P(1),P(3),'o','color','k');hold on;
%                     figure(fignum-2);plot3(Q(2),Q(1),Q(3),'o','color','k');hold on;
%                     figure(fignum-2);PlotElectrodes2(linePQ_sub,XYZWorldLimits,'k','-.');hold on;
%                     figure(fignum-2);plot3(C(2),C(1),C(3),'o','color','g');hold on;
                    CtoPQ=norm(cross(P-Q,C-Q))/norm(P-Q);
                    if CtoPQ<2
