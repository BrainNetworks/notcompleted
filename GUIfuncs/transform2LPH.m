...
%%关于函数的输出
    %M为从XYZ坐标系向LPH坐标系变换的4*4二维矩阵，满足(l;p;h;1)=M*(x;y;z;1)
    %LPH为输入数据全体体素在LPH坐标系中的坐标群，为4维矩阵，维度为4×n×m×s(注意不是4×m×n×s)
%% ------定义输入组图在XYZ坐标系中的初始坐标------ %%
[row_data,column_data,slice_data]=size(data); 
xyz=zeros(4,row_data,column_data,slice_data); %定义四维矩阵
...
M=TIFF*R*S*T0;
%% ------实现坐标变换------ %%
LPH=zeros(4,column_data,row_data,slice_data); %定义四维矩阵
h=waitbar(0,'Coordinate transforming...'); %创建进度条
for z=1:slice_data
    for y=1:row_data
        LPH(:,:,y,z)=M*xyz(:,:,y,z); %x为列索引，y为行索引，z为层索引
    end 
